...
import static crossover.social.media.plugin.asset.AssetUtils.findAssetTypeByFileName;
...
public class OakAssetManagementPlugin extends AbstractAssetManagementPlugin<OakContainer, OakAsset> {
    ...
    protected void createSettings() throws PluginOperationException {
        ...
        settings.add(new Setting(getCompoundKey("dbName"), getSetting("dbName", String.class, "<change me>"), SettingType.TEXT));
        settings.add(new Setting(getCompoundKey("dbHost"), getSetting("dbHost", String.class, "<change me>"), SettingType.TEXT));
        settings.add(new Setting(getCompoundKey("dbPort"), getSetting("dbPort", Integer.class, 0), SettingType.INTEGER));
        settings.add(new Setting(getCompoundKey("username"), getSetting("username", String.class, "<change me>"), SettingType.TEXT));
        settings.add(new Setting(getCompoundKey("password"), getSetting("password", String.class, "<change me>"), SettingType.TEXT));
    }
    ...
    protected void loadChildren(String siteId, OakContainer siteRepository) throws PluginOperationException {
        try {
            ...
            for (Node node : childNodes) {
                SocialAsset socialAsset = new SocialAsset(siteId, node.getName(), new Date(), node.getName(), String.format("%s/%s", siteId, node.getName()));
                socialAsset.setType(findAssetTypeByFileName(node.getName()));

                assetRepository.save(socialAsset);
            }
